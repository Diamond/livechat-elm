...
Elm.LiveChat.make = function (_elm) {
   ...
   var actions = A2($Signal.merge,inbox.signal,A2($Signal.map,Add,incomingMessages));
   ...
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return model;
         case "Add": return _U.update(model,{messages: A2($Basics._op["++"],model.messages,_U.list([_p1._0]))});
         case "Update": return _U.update(model,{newMessage: _p1._0});
         default: return _U.update(model,{messages: A2($Basics._op["++"],model.messages,_U.list([messageFromModel(model)])),newMessage: "",broadcast: true});}
   });
   var initialModel = {author: "Guest1234",newMessage: "",messages: _U.list(["<Guest> Hello World!","<Guest2> You would say that!"]),broadcast: false};
   ...
   function () {
      var lastMessage = function (messages) {    return $List.head($List.reverse(messages));};
      return $Signal.dropRepeats(A2($Signal.map,function (m) {    return lastMessage(m.messages);},model));
   }());
   var main = A2($Signal.map,view(inbox.address),model);
   var Model = F4(function (a,b,c,d) {    return {author: a,newMessage: b,messages: c,broadcast: d};});
